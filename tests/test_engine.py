...
class TestEngineConstructor:
    ...
    def test_pool_kwargs(self):
        eng = Engine(
            sqlite3, _connect,
            pool_kwargs={'limit': 5, 'timeout': 3.0},
        )
...
class TestEngineQuery:
    ...
    def test_static_by_default(self, standalone_engine):
        eng = Engine(
            sqlite3, _connect,
            str_templates_static_by_default=True,
        )
...
class TestEngineQueryFrom:
    ...
    def test_raises_on_unknown_extension(self, engine):
        with pytest.raises(
            ValueError, match='Unsupported filename extension',
        ):
...
class TestEngineTransaction:
    ...
    def test_commits_on_success(self, standalone_engine):
        with standalone_engine.transaction():
            standalone_engine.query(
                'CREATE TABLE _tx_comm (id INT)', static=True,
            ).execute()
            standalone_engine.query(
                'INSERT INTO _tx_comm (id) VALUES (:v)', static=True,
            ).execute(v=1)
        with standalone_engine.transaction():
    ...
    def test_rollback_on_exception(self, standalone_engine):
        with standalone_engine.transaction():
            standalone_engine.query(
                'CREATE TABLE _tx_rb (id INT)', static=True,
            ).execute()
        try:
            with standalone_engine.transaction():
                standalone_engine.query(
                    'INSERT INTO _tx_rb (id) VALUES (:v)', static=True,
                ).execute(v=2)
    ...
    def test_commit_false_rolls_back(self, standalone_engine):
        with standalone_engine.transaction():
            standalone_engine.query(
                'CREATE TABLE _tx_nc (id INT)', static=True,
            ).execute()
        with standalone_engine.transaction(commit=False):
            standalone_engine.query(
                'INSERT INTO _tx_nc (id) VALUES (:v)', static=True,
            ).execute(v=3)
        with standalone_engine.transaction():
            result = standalone_engine.query(
    ...
    def test_nested_transactions_create_savepoint(
        self, standalone_engine,
    ):
        with standalone_engine.transaction():
            standalone_engine.query(
                'CREATE TABLE _tx_nest (id INT, val TEXT)', static=True,
            ).execute()
            standalone_engine.query(
                'INSERT INTO _tx_nest (id, val) VALUES (:i, :v)',
                static=True,
            ).execute(i=1, v='outer')
            with standalone_engine.transaction():
                standalone_engine.query(
    ...
    def test_nested_rollback_to_savepoint(self, standalone_engine):
        with standalone_engine.transaction():
            standalone_engine.query(
                'CREATE TABLE _tx_nrb (id INT)', static=True,
            ).execute()
            standalone_engine.query(
                'INSERT INTO _tx_nrb (id) VALUES (:v)', static=True,
            ).execute(v=1)
            try:
                with standalone_engine.transaction():
                    standalone_engine.query(
                        'INSERT INTO _tx_nrb (id) VALUES (:v)',
                        static=True,
                    ).execute(v=2)
    ...
    def test_nested_decorator(self, standalone_engine):
        ...
        def outer():
            standalone_engine.query(
                'CREATE TABLE _tx_dec (id INT)', static=True,
            ).execute()
            standalone_engine.query(
                'INSERT INTO _tx_dec (id) VALUES (:v)', static=True,
            ).execute(v=10)
