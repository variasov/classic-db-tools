...
from classic.db_tools import Engine, ToCls, ToDict, OneToMany
...
def tasks(engine: Engine, ddl):
    ...
    engine.queries.example.save_task_statuses([
        {'status': 'CREATED', 'task_id': 1},
        {'status': 'CREATED', 'task_id': 2},
        {'status': 'CREATED', 'task_id': 3},
        {'status': 'STARTED', 'task_id': 1},
        {'status': 'FINISHED', 'task_id': 1},
    ])
    yield


def test_returning_with_cls(engine: Engine, ddl, tasks):
...
def test_returning_with_dict(engine: Engine, ddl, tasks):
    assert engine.from_str(query).execute(
    ).returning(
        ToDict('Task'),
        ToDict('TaskStatus'),
