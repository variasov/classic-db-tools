...
class Engine:
    ...
    Для доступа к разным инстансам БД следует использовать
    разные инстансы Engine.

    Позволяет совершать запросы к БД через DB API 2.0 совместимый драйвер.
    Под капотом содержит пул коннектов.

    Дает совершать запросы через Engine.query() и Engine.query_from().
    При вызове этих методов движок самостоятельно соединение в пуле,
    выполнит запрос и вернет соединение обратно.
    По умолчанию соединение принудительно переводится в режим autocommit.

    Для удержания коннекта из пула для совершения нескольких запросов
    используйте метод Engine.conn(), возвращающий контекстный менеджер,
    на выходе возвращающий коннект обратно. Во время работы контекста
    соединение так же доступно в поле engine.current.conn.

    Для работы с транзакциями используйе метод Engine.transaction(), также
    возвращающий контекстный менеджер. Во время работы транзакций режим
    autocommit отключается.

    Вложенные транзакции с разными параметрами не поддерживаются,
    и вызовут ошибку.
    """
    ...
    def transaction(
        ...
        **tx_kwargs: Any,
        ...
        При вызове engine.transaction в другом блоке transaction произойдет
        создание SAVEPOINT, при выходе из внутреннего произойдет удаление
        SAVEPOINT.
